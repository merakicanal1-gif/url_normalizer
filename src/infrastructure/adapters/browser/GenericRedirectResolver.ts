...
export class GenericRedirectResolver implements IUrlResolver {
  constructor(
  ...
  public async resolve(url: URL, _timeoutMs?: number): Promise<ResolvedUrl> {
    ...
    try {
      ...
      return {
        ...
        outcome,
        metadata: {
          resolver: 'GenericRedirectResolver',
          strategy: 'http',
      ...
      const durationMs = Math.round(performance.now() - start);
      this.logger.error(`[GenericRedirectResolver] Erro na resolução HTTP ou loop detectado. Delegando via CONTINUE...`, err);
