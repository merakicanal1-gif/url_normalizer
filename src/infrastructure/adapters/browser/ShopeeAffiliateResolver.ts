...
export class ShopeeAffiliateResolver implements IUrlResolver {
  constructor(
    private logger: { info: (msg: string) => void; error: (msg: string, err?: any) => void }
  ) {}

  public canResolve(url: URL): boolean {
    return MarketplaceHostRegistry.isShopeeAffiliate(url.hostname);
  }
  ...
  public async resolve(url: URL, _timeoutMs?: number, profileId?: string, sessionPage?: INavigatorPage): Promise<ResolvedUrl> {
    ...
    try {
      ...
      this.logger.info(`[ShopeeAffiliateResolver] Resolução HTTP concluída em ${durationMs}ms com ${res.redirects.length} redirecionamentos. ResolvedSuccess: ${res.resolvedSuccess}`);
      ...
      return {
        ...
        outcome,
        metadata: {
          resolver: 'ShopeeAffiliateResolver',
          strategy: 'http',
      ...
      const durationMs = Math.round(performance.now() - start);
      this.logger.error(`[ShopeeAffiliateResolver] Erro na resolução HTTP. Delegando via CONTINUE...`, err);
